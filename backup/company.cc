...
double Company::get_investment_cashflow(double items, double loans) {
  ...
  while (capacity_ <= new_capacity) {
    production = get_prod(skill_sum, prod_const_skill_, mot_sum, prod_const_motivation_, size, new_capacity);
...
void Company::pay_employees() {
  ...
  //cout << name_ << "I company pay employ1" << endl;  
  ...
  //cout << name_ << "    " << *kukballe << "     I company pay employ2: " << wage << endl;  
